...
/**
 * Create hyperframe for compound hypercube - two complete hypercube hyperframes
 * Each hypercube gets:
...
export function createCompoundHypercubeHyperframe(
  ...
  console.log("Creating compound hypercube hyperframe (2 hypercubes)");
  ...
  // Create hyperframe for both hypercubes
  for (let hypercubeIndex = 0; hypercubeIndex < 2; hypercubeIndex++) {
    // Apply rotation for second hypercube (45° on Y axis)
    const rotation =
      hypercubeIndex === 0
    ...
    const outerVertices = cubeVertices.map((v) => {
      ...
      if (hypercubeIndex === 1) {
        ...
        vec.y += 0.01; // Slight offset to prevent z-fighting
      }
  ...
  console.log(
    `Created compound hypercube inner wireframes: ${
      cubeEdges.length * 2
    } cylinder edges (12 per hypercube)`
  );
  console.log(
    `Created compound hypercube connections: ${
      8 * 2
