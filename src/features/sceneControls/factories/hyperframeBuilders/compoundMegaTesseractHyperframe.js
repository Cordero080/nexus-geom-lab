...
export function createCompoundMegaTesseractHyperframe(
  ...
  const innerSize1 = 0.375 * scale; // Inner cube for first pair
  const innerSize2 = 1.5 * scale;   // Inner cube for outer pair
  ...
  const rotCross1 = new THREE.Matrix4().multiplyMatrices(rotCross1X, rotCross1Y);
  const cross1Inner = rotateVertices(makeCubeCorners(innerSize1), rotCross1);

  // Cross 2: Y rotation π/6 + π/4, then X rotation π/2
  const rotCross2Y = new THREE.Matrix4().makeRotationY(Math.PI / 6 + Math.PI / 4);
  const rotCross2X = new THREE.Matrix4().makeRotationX(Math.PI / 2);
  const rotCross2 = new THREE.Matrix4().multiplyMatrices(rotCross2X, rotCross2Y);
  ...
  const rotCross3 = new THREE.Matrix4().multiplyMatrices(rotCross3Z, rotCross3Y);
  const cross3Inner = rotateVertices(makeCubeCorners(innerSize2), rotCross3);

  // Cross 4: Y rotation π/5 + π/4, then Z rotation π/2
  const rotCross4Y = new THREE.Matrix4().makeRotationY(Math.PI / 5 + Math.PI / 4);
  const rotCross4Z = new THREE.Matrix4().makeRotationZ(Math.PI / 2);
  const rotCross4 = new THREE.Matrix4().multiplyMatrices(rotCross4Z, rotCross4Y);
  ...
  console.log(`Created 8 inner cube wireframes (12 edges × 8 cubes = 96 edges)`);
