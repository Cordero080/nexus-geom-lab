...
export function createCompoundHypercube(options = {}) {
  ...
  // First hypercube with connecting frustum faces
  const hypercube1 = createHypercubeWithFaces(outerScale, innerScale, null);

  // Second hypercube rotated 45° on Y axis (simulates 4D rotation)
  const rotation = new THREE.Euler(0, Math.PI / 4, 0);
  const hypercube2 = createHypercubeWithFaces(outerScale, innerScale, rotation);
  hypercube2.translate(0, 0.01, 0); // Slight offset to prevent z-fighting

  // Merge both complete hypercubes
  console.log("Attempting to merge compound hypercube parts");
  const mergedCpdHypercube = mergeGeometries([hypercube1, hypercube2]);

  if (!mergedCpdHypercube) {
    console.error("Failed to merge compound hypercube geometries!");
    return hypercube1;
  }

  console.log("Successfully merged compound hypercube");
...
export const metadata = {
  ...
  displayName: "◻◻ Compound Hypercube",
  category: "polytopes",
