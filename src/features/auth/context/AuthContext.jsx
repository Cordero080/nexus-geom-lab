...
export function AuthProvider({ children }) {
  ...
  const isAnimationUnlocked = useCallback(
    (noetechKey, animationId) => {
  ...
  const getUnlockedAnimationsForNoetech = useCallback(
    (noetechKey) => {
  ...
  const addUnlockedNoetechs = useCallback((newKeys) => {
    ...
    setUser((prev) => {
      ...
      const prevList = Array.isArray(prev.unlockedNoetechs)
        ? // Get existing unlocks : []
          prev.unlockedNoetechs
        : [];
